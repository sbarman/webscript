  ...
  Replay.prototype = {
    ...
    simultaneousReplayGuts: function _simultaneousReplayGuts(e) {
      ...
      //no twin port yet, but we have a twin tab
        ...
        var newPort = this.replayFindPortInTab(desiredTab, e.topFrame, e.snapshot, e.msg);
        if (newPort) {
      ...
      //making a tab
        ...
        if(!this.makingTab){
          ...
          //tab
          this.makingTab = true;
          chrome.tabs.create({windowId: this.twinWindow, url: e.topURL, active: true}, 
            function(newTab) {
              var newTabId = newTab.id;
...
var simultaneousReplayer = new Replay([],panel,ports);
